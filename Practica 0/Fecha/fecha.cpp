...
Fecha::Fecha (int dia, int mes, int anno) throw (Fecha::Invalida)
{
	...
	if(fechainvalida(d,m,a))
		throw Invalida("Error Fecha Invalida en constructor Fecha");
	else
	{
		dia_ = d;
		mes_ = m;
...
Fecha::Fecha (const char *fecha) throw (Invalida)
{
	...
	{
		if(fechainvalida(d,m,a))
			throw Invalida("Error Fecha Invalida en constructor const char* -> Fecha");
		else
		{
			dia_ = d;
			mes_ = m;
...
Fecha::operator const char* () const
{
	...
	static char cadenafecha[50];// = new char[50];
	//boost::shared_ptr<char> cadenafecha(new char[50]);	
	...
	td->tm_mday = dia_;
	td->tm_mon = mes_-1;
	td->tm_year = anno_-1900;
	
	// Para calcular el dia de la semana.
	mktime(td);	
...
bool operator <(const Fecha& f, const Fecha& g)
{
	return (f.anno() < g.anno()) || ((f.anno() == g.anno()) && (f.mes() < g.mes())) || 
			((f.anno() == g.anno()) && (f.mes() == g.mes()) && (f.dia() < g.dia()));
}
